class InputParser {
  ...
  * parse() {
    for (const item of this.data) {
      let range, character, password;
      [range, character, password] = item.split(' ');
      ...
      const condition = new Condition(character, from, to);
...
class Condition {
  constructor(requiredChar, min, max) {
...
class PositionValidator {
  ...
  valid() {
    ...
    return firstPosition !== secondPosition &&
      (firstPosition === this.condition.requiredChar || secondPosition === this.condition.requiredChar)
  }
}
...
export default {
  part1() {
    ...
    for (let [password, condition] of parser.parse()) {
